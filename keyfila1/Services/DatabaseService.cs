...
namespace keyfila1.Services
{
    ...
    {
        // Connection string for the server (without database) to create it if it doesn't exist
        private static readonly string ServerConnectionString = "Server=localhost;User ID=root;Password=root;";
        // Connection string for the specific database
        private static readonly string ConnectionString = "Server=localhost;Database=keyfila_db;User ID=root;Password=root;";
        ...
        public static void InitializeDatabase()
        {
            try
            {
                ...
                {
                    conn.Open();
                    conn.Execute("CREATE DATABASE IF NOT EXISTS keyfila_db;");
                }

                // 2. Ensure Tables exist and have data
                using (var conn = GetConnection())
                {
                    ...
                    // UserProgress Table (Updated with UserId and Stars)
                    conn.Execute(@"
                    ...
                    try { conn.Execute("ALTER TABLE UserProgress ADD COLUMN UserId INT;"); } catch { }
                    try { conn.Execute("ALTER TABLE UserProgress ADD COLUMN Stars INT DEFAULT 0;"); } catch { }
                    try { conn.Execute("ALTER TABLE UserProgress ADD CONSTRAINT fk_user FOREIGN KEY (UserId) REFERENCES Users(Id);"); } catch { }
                    
                    // Add security columns to Users if they don't exist
                    try { conn.Execute("ALTER TABLE Users ADD COLUMN SecurityQuestion VARCHAR(255);"); } catch { }
                    try { conn.Execute("ALTER TABLE Users ADD COLUMN SecurityAnswer VARCHAR(255);"); } catch { }

                    // 3. Check if we need to add initial lessons (Seed data)
                    int count = conn.ExecuteScalar<int>("SELECT COUNT(*) FROM Lessons");
                    // FORCED UPDATE: Changed to 'true' to ensure your database gets the new 50+ word lessons
                    if (true) 
                    {
                        conn.Execute("DELETE FROM UserProgress;"); // Clear old stats to avoid FK issues
                        ...
                        conn.Execute(@"
                            ...
                            -- Intermediate (Difficulty 1) - Minimized (15-17 Words)
                            ...
                            -- Advanced (Difficulty 2) - Minimized Paragraphs (15-17 Words)
            ...
            {
                throw new Exception($"Database Setup Failed.\n\nError: {ex.Message}\n\nHint: Check if MySQL is running and your 'root' password is correct in DatabaseService.cs.");
            }
        }
